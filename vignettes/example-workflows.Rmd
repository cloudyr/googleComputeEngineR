...
The following are some R scripts for common workflows.  They assume you have previously signed up and setup a Google project and authentication.
...
Here we are setting up a 13GB RAM instance, as found via `gce_list_machinetypes()`
...
Instead of the more generic `gce_vm()` that is used for more interactive use, we create the instances directly using `gce_vm_container` so it doesn't wait for the job to complete before starting the next. You can then use `gce_get_zone_op` to get the job status. 
...
It is safest to setup the SSH keys seperately for multiple instances:
...
The cluster is now ready to recieve jobs.  You can send them up my simply using the `%<-%` operator instead of `<-`
...
Remember to shut down your cluster.  You are charged per minute of uptime. 
...
vm <- gce_vm(name = "build-schedule-r", template = "rstudio-hadleyverse", predefined_type = "n1-standard-2")
```
...
It is shown below, which you could base your own upon.  This one installs `cron` for scheduling, and `nano` a simple text editor. It then also installs `googleAuthR` and `shinyFiles` from CRAN, as these are needed for my scripts, and from Github `bnosac/cronR` to help with creating cron jobs within RStudio. 
...
You can now launch instances using your constructed image.  You can even use your custom image to create further `Dockerfiles` that use it as a dependency, using `gce_tag_container` to get its correct name. 

```r
## now start an instance using our rstudio image in cloud-config
## this takes care of rstudio friendly settgings, restart behaviour etc.
## get instance tags via gce_tag_container() 
...
## Deploying a self-contained Shiny app in Docker

This like the above example uses a local Dockerfile to install the libraries you need for your Shiny app, 
but then also uploads a Shiny app into the custom image so its all self-contained. 
...
The Docker file is using a `googleAuthR` demo app, and is found via: `get_dockerfolder("shiny-googleAuthRdemo")`
...
```
## upload local Dockerfile that will build the upload app 
...
You can now deploy your Shiny app on any instance by calling it from the Container Registry
...
Clean up the VMs to avoid unecessary costs:
...
You have a few options with OpenCPU for deploying, including just calling directly from a github repo.

But the below installs your Github repo into the Docker container using a small Dockerfile as detailed above.
...
4. Stop the default OpenCPU docker and launch your own
