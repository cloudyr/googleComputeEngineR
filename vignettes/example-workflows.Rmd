...
It is not recommended to rely on loading data into a Docker contianer.  Instead, call outside dedicated data stores such as BigQuery or Cloud Storage, which if you are using Google Compute Engine you have access to under the same project. 
...
If you want to call the data from a Shiny app, then you can use `bqr_query` from bigQueryR or `gcs_get_object` from googleCloudStorageR within your app's `server.R` to pull in the data into your app. 
