...
The master needs to only have enough to create other VMs and download files from Google Cloud Storage:
...
The slave needs to be have enough R libraries to run your scheduled script, plus `googleCloudStorageR` to download/upload the auth files and script.  An example that includes `googleAnalyticsR`, `searchConsoleR`and others is found at `system.file("dockerfiles", "gceScheduler", "slave","Dockerfile", package = "googleComputeEngineR")`
