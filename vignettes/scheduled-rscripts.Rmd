...
3. Launch a VM that will run your scheduled scripts - keep this live
4. Fetch that VM via `gce_vm_scheduler`
...
The above script can then be scheduled via:
...
The dockerfile need not be an R related one, any Docker image can be scheduled using `gce_schedule_docker`
