...
> There is currently an issue on Windows for all SSH based commands since Windows does not come with a native SSH client.  This is being looked at and a fix is pending, track progress at this [Github issue #35](https://github.com/cloudyr/googleComputeEngineR/issues/35).  Without SSH, all the `docker_x` commands will fail.  It is recommended to either use the `gce_ssh_browser()` directly or for building docker images use the `gce_docker_build` VM template. 
