...
If everything has setup successfully, then you should be able to see the SSH commands return everything is ok:

```r
lapply(vms, function(x){
  gce_ssh(vm, paste("echo", x$name, "ssh working"))
})
```
...
This will give you a docker image name such as `gcr.io/my-project/my-r` - use a version of the code below to use this in your cluster.  The `--net=host` should be included so the Docker image uses the SSH ports the host VM has (e.g. it can connect to your SSH commands)

```r
plan(cluster, workers = as.cluster(
  vms, 
  docker_image="gcr.io/my-project/my-r",
  rscript=c("docker", "run", "--net=host",
			"gcr.io/my-project/my-r", 
			"Rscript")))
```
