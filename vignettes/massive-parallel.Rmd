...
We are now ready to use all 24 cores of the cluster. For this particular example, I'm going to loop over 48 iterations of our `slow_square()` function. (Note that this would take two minutes to run sequentially.) I'll use `future.lapply::future_sapply()` to run things in parallel will also use the `tictoc` package to record timing. I'm going to comment the next code block quite extensively, but just to quickly highlight the key conceptual stages:
...
ans <-
   ...
   future_sapply(seq_along(vms_nested), function(v) {
      ...
      future_vapply(input_chunk, slow_square)
   })
tictoc::toc()
#11.451 sec elapsed
